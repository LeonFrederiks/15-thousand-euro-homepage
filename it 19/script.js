...
class PixelCanvas {
    ...
    resizeCanvas() {
        ...
        if (this.isFullscreen) {
            // In fullscreen mode, fit canvas with 16px padding
            const availableWidth = containerWidth - 32; // 16px padding on each side
            const availableHeight = containerHeight - 32; // 16px padding on top/bottom
            
            // Calculate scale to fit while maintaining 1920x1080 ratio
            const scaleX = availableWidth / this.ORIGINAL_WIDTH;
            const scaleY = availableHeight / this.ORIGINAL_HEIGHT;
            ...
            // In normal mode, maintain aspect ratio within container
            const availableWidth = Math.min(containerWidth - 64, 1200); // Use max 1200px width
            const aspectRatio = 1920 / 1080;
            const calculatedHeight = availableWidth / aspectRatio;
            
            // Ensure canvas doesn't exceed container height
            const maxHeight = Math.min(containerHeight * 0.8, 675); // Use 80% of container height as max, but max 675px
            ...
            const scale = finalWidth / this.ORIGINAL_WIDTH;
    ...
    updateCursorRectangle(x, y) {
        if (!this.cursorRectangle || this.isFullscreen) return;
        
        // Show cursor rectangle
        this.cursorRectangle.classList.remove('hidden');
        
        // Position rectangle relative to canvas
        const canvasRect = this.canvas.getBoundingClientRect();
        const relativeX = (x / this.ORIGINAL_WIDTH) * this.canvas.width;
        const relativeY = (y / this.ORIGINAL_HEIGHT) * this.canvas.height;
        
        this.cursorRectangle.style.left = `${canvasRect.left + relativeX}px`;
        this.cursorRectangle.style.top = `${canvasRect.top + relativeY}px`;
    ...
    showTooltip(clientX, clientY, pixelData) {
        ...
        // Calculate tooltip content based on pixel data
        const pixelInfo = `
            <div><strong>${pixelData.message || 'Purchased Pixel'}</strong></div>
            <div>${pixelData.width}×${pixelData.height} pixels (€${pixelData.price})</div>
            ${pixelData.link ? `<div><a href="${pixelData.link}" target="_blank">Visit Site</a></div>` : ''}
        `;
        tooltip.innerHTML = pixelInfo;
        
        // Get canvas position and current scale to position tooltip next to pixel
        const canvasRect = this.canvas.getBoundingClientRect();
        const gridX = Math.floor((clientX - canvasRect.left) / this.scale);
        const gridY = Math.floor((clientY - canvasRect.top) / this.scale);
        
        // Position tooltip to the right and slightly below the pixel
        let tooltipX = canvasRect.left + (gridX + 1) * this.scale + 10;
        let tooltipY = canvasRect.top + gridY * this.scale + 5;
        ...
        if (tooltipX + 200 > window.innerWidth) {
            tooltipX = canvasRect.left + gridX * this.scale - 210; // Place to the left
        }
        
        // Check if tooltip would go off screen vertically
        if (tooltipY + 80 > window.innerHeight) {
            tooltipY = window.innerHeight - 90; // Place at bottom
        }
        
        // Ensure tooltip stays within viewport
        tooltipX = Math.max(10, Math.min(tooltipX, window.innerWidth - 210));
        tooltipY = Math.max(10, Math.min(tooltipY, window.innerHeight - 90));
    ...
    renderMockData() {
        this.purchasedPixels.forEach(pixel => {
            ...
            const pixelColor = pixel.color || '#007AFF';
            ...
            // Draw border
            this.ctx.strokeStyle = pixelColor;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    setTimeout(() => {
        if (window.pixelCanvas) {
            window.pixelCanvas.renderCanvas();
        }
    }, 100);
});
